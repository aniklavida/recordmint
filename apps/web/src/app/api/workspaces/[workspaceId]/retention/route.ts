...
export async function PATCH(request: Request, { params }: { params: { workspaceId: string } }): Promise<Response> {
  try {
    const user = await requireUser();
    await requireMembership({ workspaceId: params.workspaceId, userId: user.id, minimumRole: "owner" });
    const body = (await request.json()) as { retentionDays: number | null };
    const updated = await setWorkspaceRetention(params.workspaceId, body.retentionDays);
